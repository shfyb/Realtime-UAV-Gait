...
root = Path('.')
checks = [
    root / 'demo/checkpoints/Drone-YOLO/best.pt',
    root / 'demo/checkpoints/gait_model/GaitBase_DronGait1-60000.pt',
    root / 'demo/checkpoints/seg_model/human_pp_humansegv2_mobile_192x192_inference_model_with_softmax/model.pdiparams',
]
for p in checks:
...
If you migrated from a local backup bundle, copy the three weight files from your old `OpenGait/demo/checkpoints/` tree.
