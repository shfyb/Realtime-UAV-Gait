...
from demo.libs.paddle.infer import Predictor_opengait
...
def load_seg_model(config, gpu_id=0, use_gpu=True):
    """一次性加载分割模型"""
    predictor = Predictor_opengait(config, gpu_id=gpu_id, use_gpu=use_gpu)
    return predictor

def predict_seg(predictor, img, save_path):
    """用已加载的 predictor 进行分割并保存"""
    bg_img = 255 * np.ones(img.shape, dtype=np.uint8)
    out_img, out_mask = predictor.run(img, bg_img)

    # 阈值二值化
    threshold = 80
    out_mask = np.where(out_mask < threshold, 0, 255).astype(np.uint8)
