...
track_cfgs = {  
    "model":{
        ...
        #"fair_ckpt": "./demo/checkpoints/fairmot_model/fairmot_lite.pth"
        #"fair_ckpt": "./demo/checkpoints/fairmot_model/fairmot_hrnet_w18.pth" 
        #"fair_ckpt": "./demo/checkpoints/fairmot_model/model_15_fair.pth"   
        #"fair_ckpt": "./demo/checkpoints/fairmot_model/mot20_fairmot.pth"
    },
    "gait":{
        "dataset": "GREW",
    },
...
def track(video_path, video_save_folder):
    ...
    Returns:
        track_results (dict): Track information, only includes the most frequent ID renamed to 001
    """
    ...
    for i in tqdm(range(frame_count)):
        ...
        for t in online_targets:
            ...
            if mark:
                mark = False
                diff = tid - 1
            tid = tid - diff  # Normalize ID to start from 1
    ...
    # ====== 保留出现最多帧数的 ID 并重命名为 001 ======
    id_frame_count = defaultdict(int)
    for frame_data in raw_track_results.values():
        for obj in frame_data:
            id_frame_count[obj[0]] += 1

    if not id_frame_count:
        print("⚠️ 没有检测到任何目标！")
        return {}

    most_common_id = max(id_frame_count.items(), key=lambda x: x[1])[0]
    ...
    for frame_id, objs in raw_track_results.items():
        for obj in objs:
            if obj[0] == most_common_id:
                new_obj = [1, obj[1], obj[2], obj[3], obj[4]]  # ID 替换为 1
    ...
    # ====== 保存跟踪结果为 txt（可选）======
    if track_cfgs.get("save_result", "False") == "True":
        res_file = osp.join(video_save_folder, f"{save_video_name}.txt")
        with open(res_file, 'w') as f:
            f.writelines(results)

    return track_results  # 最终只返回 ID 为 001 的目标



'''
##  多目标追踪，用原始的Fairmot_dla34.pth 0.5 0.5
def track(video_path, video_save_folder):
    frame_id = 0
